...
class RSSGenerator:
    def __init__(self, settings: dict):
        ...
        self.base_url = os.getenv(
            "PODCAST_BASE_URL", "https://example.github.io/grawth-podcast"
        )
        self.title = os.getenv("PODCAST_TITLE", "グロース市場ニュース")
        self.description = os.getenv(
            "PODCAST_DESCRIPTION",
            "東証グロース市場の最新動向を毎朝お届けする音声ニュースです。教育目的であり投資助言ではありません。",
        )
        self.author = os.getenv("PODCAST_AUTHOR", "grawth-podcast")
        ...
        self.category = podcast_cfg.get("category", "Business")
        self.subcategory = podcast_cfg.get("subcategory", "Investing")
    ...
    def _add_channel_metadata(self, channel: Element) -> None:
        ...
        SubElement(channel, "generator").text = "Grawth Podcast Generator"

        SubElement(channel, "{%s}author" % ITUNES_NS).text = self.author
        SubElement(channel, "{%s}owner" % ITUNES_NS).append(
            self._make_itunes_owner()
        )
    ...
    def _add_episode_item(self, channel: Element, episode: dict) -> None:
        ...
        title = episode.get("title", f"{date_str}のグロース市場ニュース")
        ...
        SubElement(item, "guid", isPermaLink="false").text = f"grawth-podcast-{date_str}"
        ...
        if episode_num:
            SubElement(item, "{%s}episode" % ITUNES_NS).text = str(episode_num)
