#!/usr/bin/env python3
"""グロース市場Podcast自動生成システム - メインオーケストレーター"""
...
def _load_companies(settings: dict) -> list[dict]:
...
def _get_paths(settings: dict, target_date: str) -> dict:
    data_dir = BASE_DIR / settings.get("system", {}).get("data_dir", "data")
...
def run(target_date: str, settings: dict, dry_run: bool = False) -> bool:
    ...
    logger.info(f"=== グロース市場Podcast生成開始: {target_date} ===")
    ...
    # ── 企業リストをDB登録 ────────────────────────────────────────────────
    companies = _load_companies(settings)
    repo.upsert_companies(companies)
    logger.info(f"監視企業数: {len(companies)}社")

    # ── Step 1: 市場データ収集 ─────────────────────────────────────────
    logger.info("Step 1: 市場データ収集")
    from .collectors.market_data import MarketDataCollector
    market_collector = MarketDataCollector(settings)

    market_data = market_collector.fetch_index_data(target_date)
    if market_data:
        repo.upsert_market_data(market_data)
        logger.info(f"指数データ保存: 終値={market_data.get('close_price')}")
    else:
        logger.warning("指数データ取得失敗")

    stock_data_list = market_collector.fetch_all_stocks(
        repo.get_all_companies(), target_date
    )
    for sd in stock_data_list:
        repo.upsert_market_data(sd)

    breadth = market_collector.calculate_market_breadth(stock_data_list)
    if market_data:
        market_data.update(breadth)
        repo.upsert_market_data(market_data)

    # ── Step 2: ニュース収集 ───────────────────────────────────────────
    logger.info("Step 2: ニュース収集")
    from .collectors.news_collector import NewsCollector
    news_collector = NewsCollector(settings)
    all_news = news_collector.fetch_all(repo.get_all_companies(), target_date)
    repo.upsert_news(all_news)

    # ── Step 3: IPO情報収集 ────────────────────────────────────────────
    logger.info("Step 3: IPO情報収集")
    from .collectors.ipo_collector import IPOCollector
    ipo_collector = IPOCollector()
    ipo_text = ipo_collector.get_upcoming_ipos_text(days_ahead=7)
    logger.info(f"IPO: {ipo_text[:80]}...")

    # ── Step 4: AI分析 ─────────────────────────────────────────────────
    logger.info("Step 4: AI分析")
    from .analyzers.ai_analyzer import AIAnalyzer
    analyzer = AIAnalyzer(settings)

    fresh_market_data = repo.get_market_data(target_date, "index")
    fresh_stock_data = repo.get_stock_data_for_date(target_date)
    fresh_news = repo.get_news_for_date(target_date, limit=30)

    if not dry_run:
        analysis = analyzer.analyze(
            target_date, fresh_market_data, fresh_stock_data, fresh_news, ipo_text
        )
        repo.save_analysis(target_date, analysis)
    else:
        logger.info("Dry run: AI分析をスキップ")
        analysis = {
            "market_summary": "Dry run - AI分析スキップ",
            "spotlight_companies": [],
            "theme_analysis": "Dry run",
            "tomorrow_points": "Dry run",
            "trending_themes": [],
            "keywords": [],
            "model_used": "dry_run",
        }
    ...
    # ── Step 5: トレンド分析JSON生成 ──────────────────────────────────
    logger.info("Step 5: トレンド分析JSON生成")
    from .analyzers.trend_analyzer import TrendAnalyzer
    trend_analyzer = TrendAnalyzer(repo)
    trend_data = trend_analyzer.generate_analysis_json(target_date)
    with open(paths["analysis_json"], "w", encoding="utf-8") as f:
        json.dump(trend_data, f, ensure_ascii=False, indent=2)
    logger.info(f"analysis.json 保存: {paths['analysis_json']}")

    # ── Step 6: 台本生成 ───────────────────────────────────────────────
    logger.info("Step 6: 台本生成")
    from .generators.script_generator import ScriptGenerator
    script_gen = ScriptGenerator(settings)
    script = script_gen.generate(
        target_date, fresh_market_data, fresh_stock_data,
        fresh_news, analysis, ipo_text
    )
    ...
    file_size = Path(paths["podcast_mp3"]).stat().st_size if Path(paths["podcast_mp3"]).exists() else 0
    ...
    # ── Step 11: 古いMP3削除 ───────────────────────────────────────────
    logger.info("Step 11: 古いMP3削除")
    _cleanup_old_mp3(
        paths["episodes_dir"],
        keep_days=settings.get("podcast", {}).get("max_mp3_storage_days", 7)
    )

    # ── Step 12: ボイスファイル削除（クリーンアップ設定による）─────────
    if settings.get("cleanup", {}).get("delete_raw_wav", True):
        raw_voice = Path(paths["voice_mp3"])
        if raw_voice.exists():
            raw_voice.unlink()
            logger.info(f"中間ファイル削除: {paths['voice_mp3']}")

    logger.info(f"=== 完了: {target_date} (所要時間: 生成済み) ===")
    return True
...
def _cleanup_old_mp3(episodes_dir: str, keep_days: int = 7) -> None:
    ...
    deleted = 0
    for mp3_file in episodes_path.glob("podcast_*.mp3"):
    ...
    if deleted:
        logger.info(f"{deleted}件の古いMP3を削除")
    else:
        logger.info("削除対象のMP3なし")


def main():
    parser = argparse.ArgumentParser(description="グロース市場Podcast生成システム")
    parser.add_argument(
        "--date",
