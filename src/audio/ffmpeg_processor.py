...
class FFmpegProcessor:
    def __init__(self, settings: dict, assets_dir: str = "assets"):
        ...
        self.opening_fade_in_ms = audio_cfg.get("opening_fade_in_ms", 500)
        self.ending_fade_out_ms = audio_cfg.get("ending_fade_out_ms", 2000)
    ...
    def process(self, voice_mp3: str, output_mp3: str,
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                self._add_opening_ending(
                    str(normalized), str(with_opening_ending), tmpdir
                )

                with_bgm = tmp / "with_bgm.mp3"
                bgm_added = self._mix_bgm(
                    str(with_opening_ending), str(with_bgm)
                )
                ...
                self._export_mp3(
                    str(final_normalized), output_mp3,
    ...
    def _add_opening_ending(self, voice_path: str, output_path: str,
                            tmpdir: str) -> None:
    ...
    def _export_mp3(self, input_path: str, output_path: str,
        ...
        cmd = [
            ...
            "-metadata", f"artist=グロース市場ニュース",
            "-metadata", f"album=グロース市場ニュース",
            "-metadata", f"date={year}",
            "-metadata", f"genre=Podcast",
            "-metadata", f"comment=東証グロース市場の最新動向。教育目的。",
        ]
        if track:
